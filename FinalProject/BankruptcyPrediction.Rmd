...
actual_1_predict_1 = 1
actual_1_predict_0 = -0.0000
actual_0_predict_1 = 0.9565
actual_0_predict_0 = 1.0455
...
for (i in 1:100) {
  test_prediction_lasso=1*as.vector(validation_Probability_lasso > i/100)
...
estimation_prediction_class_lasso=1*as.vector(estimation_Probability_lasso > Probability_Threshold)
validation_prediction_class_lasso=1*as.vector(validation_Probability_lasso > Probability_Threshold)
test_prediction_class_lasso=1*as.vector(test_Probability_lasso > Probability_Threshold)

#Classification_Table=rbind(validation_data[,dependent_variable],validation_prediction_class_lasso,validation_Probability_lasso)
...
>>>>>>> 81c377952c09d4865a57ffc40526ee8ff8e669dd
...
for (i in 1:100) {
  test_prediction_class_xgboost=1*as.vector(validation_Probability_class1_xgboost > i/100)
...
ggplot(df.all, aes(x=`% of validation data selected`, y=value, colour=variable)) + scale_colour_manual(values = c("cyan", "blue", "dark green","darkred","coral","red")) + geom_line() + ylab("Estimated profit") + geom_abline(intercept = 0, slope = 1,linetype="dotted",colour="green")
```
...
rownames(test_hit_rates) <- c("Lasso Regression", "Logistic Regression","First CART", "Second CART","XGBoost","Random Forest","SVM")
...
ggplot(df.all, aes(x=`% of test data selected`, y=value, colour=variable)) + scale_colour_manual(values = c("cyan", "blue", "dark green","darkred","coral","red")) + geom_line() + ylab("Estimated profit") + geom_abline(intercept = 0, slope = 1,linetype="dotted",colour="green")
```